...
newHeadingE1.innerHTML = "Paula Gomez";
...
const validNumbersE1 = () => {
    ...
    try {
        if (!(/[0-9]/.test(valueInputE1))) { //Control that the input1 only receives numbers
        ...
        if (!(/[0-9]/.test(valueInputE2))) { //Control that the input2 only receives numbers
            inputNumberE1.style.removeProperty("border");
            document.body.appendChild(displayAlertE1);
            inputNumberE2.style.border = "3px solid red";
            throw Error("Invalid input. Please, type only numbers");
        } 
        if (!( /[0-9]/.test(valueInputE1) && /[0-9]/.test(valueInputE2))) {
            inputNumberE2.style.removeProperty("border");
            inputNumberE1.style.removeProperty("border");
            displayAlertE1.innerHTML = "Invalid input. Please, type only numbers";
            document.body.appendChild(displayAlertE1);
            inputNumberE1.style.border = "3px solid red";
            inputNumberE2.style.border = "3px solid red";
            throw Error("Invalid input3");
        }
    } catch (err) {
